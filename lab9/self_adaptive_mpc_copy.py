...
class SelfAdaptiveMPC:
    ...
    def update_step(self, dt, x_now, target_now):
        ...
        if self.predictor_type=='single_learner':
            ...
            Phi = np.zeros((3,self.n_rf))
            for i in range(self.n_rf):
                Phi[:,i] = self.Bh * cos(self.omega[i,:] * target_memory_vector + self.b[i])

            h = Phi * alpha_in / self.n_rf
            grad = -2 * Phi.T/self.n_rf * (target_now - h)
            alpha_out = alpha_in - self.lr * grad
        ...
        if self.predictor_type=='single_learner':
            ...
            for i in range(N):
                ...
                for j in range(self.n_rf):
                    Phi[:,j] = self.Bh * cos(self.omega[j,:] * target_memory_vector + self.b[j])
                h = Phi*alpha_in/self.n_rf
                target_next = h
                # # ... intermediate steps ...
                yref[:2,i] = target_next
                ...
                target_memory_vector[3:11] = target_memory_vector[0:8]
                target_memory_vector[0:2] = target_in 
