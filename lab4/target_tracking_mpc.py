...
class CrazyflieMPC(rclpy.node.Node):
    def __init__(self, node_name: str, quadrotor_dynamics: QuadrotorSimplified, mpc_N: int, mpc_tf: float, rate: int):
        ...
        # [LAB 4] Add a ROS2 subscriber for the target Crazyflie position
        ...
        # topic type -> PoseStamped
        # topic name -> {target_prefix}/pose 
        # callback -> self._target_pose_msg_callbac   
    ...
    def _twist_msg_callback(self, msg: TwistStamped):
        self.velocity = [msg.twist.linear.x,
                        msg.twist.linear.y,
                        msg.twist.linear.z]
    # [LAB 4] Implement the target position callback function (use self.target_position variable).
    ...
    # [LAB 4] Implement the trajectory type 'target_tracking' to follow the target drone.
    # Instructions:
